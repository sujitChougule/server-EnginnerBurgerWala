...
export const placeOrderOnline = asyncError(async (req, res, next) => {
  ...
  //razorpay
  const options = {
    amount: Number(totalAmount) * 100, // amount in the smallest currency unit(paisa<rupee) so multply buy 100
    currency: "INR",
  };
...
export const paymentVerification = asyncError(async (req, res, next) => {
  ...
  if (isAuthentic) {
    ...
    new ErrorHandler("Payment failed", 400);
  }
});
...
export const getOrderDetails = asyncError(async (req, res, next) => {
  ...
  if (!order) return next(new ErrorHandler("Invalid user Id", 404));
  res.status(200).json({
    success: true,
    order,
  });
...
export const prosessOrder = asyncError(async (req, res, next) => {
